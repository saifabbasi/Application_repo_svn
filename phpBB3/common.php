...
if (!defined('AVOID_AUTOLOGIN'))
{
	$user->session_begin();

	if (!isset($user->data['user_id']) || (intval($user->data['user_id'])==0) || (intval($user->data['user_id'])==1) )
	{
		...
		{	
			...
			$sql = "SELECT user_id FROM phpbb_users WHERE user_email = '{$email}'";
			$checkPhpBbUser = mysql_query($sql);
