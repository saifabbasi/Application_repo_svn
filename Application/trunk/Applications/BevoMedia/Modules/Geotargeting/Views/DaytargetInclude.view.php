...
?>
	...
	<fieldset id="landingPageFieldset_<?=$count?>" style="padding: 5px;">
		...
		<label>
			<span style="width: 120px; display: inline-block;">Daytarget:</span>
			
			<select class="formselect landingPageUrl" id="daytarget_<?=$count?>" name="daytarget_<?=$count?>_<?=isset($this->Data->ID)?$this->Data->ID:'0'?>">
			<?php 
				foreach ($this->Daytargets as $Daytarget)
				{
					$selected = '';
					
					if (isset($this->Data->DaytargetID)) {
						if ($this->Data->DaytargetID==$Daytarget->ID) $selected = 'selected';
					}
			?>
				<option value="<?=$Daytarget->ID?>" <?=$selected?>><?=$Daytarget->Name?></option>
			<?php 
				}
			?>
			</select>
