<?php

require_once('LinkTrust.Abstract.php');

/**
 * BlinkAds.php
 *
 * @category   RCS Framework
 * @package    Modules
 * @subpackage Bevomedia
 * @copyright  Copyright (c) 2009 RCS
 * @author RCS
 * @version 0.1.2
 */
class BlinkAds Extends LinkTrustAbstract {
	/**
	 * @access protected
	 * @var string $apiUrl
	 */
	protected $apiUrl = '';
	
	/**
	 * @access protected
	 * @var string $offersUrl
	 */
	protected $offersUrl = 'http://publishers.blinkads.com/RequestOffers.aspx';
	
	/**
	 * @access protected
	 * @var string $loginUrl
	 */
	protected $loginUrl = 'http://publishers.blinkads.com/Welcome/LogInAndSignUp.aspx';
	
	/**
	 * @access protected
	 * @var string $offersUsername
	 */
	protected $offersUsername = '116084';
	
	/**
	 * @access protected
	 * @var string $offersPassword
	 */
	protected $offersPassword = 'yoyoyo1025';

	/**
	 * @access protected
	 * @var string $viewState
	 */
	protected $viewState = '';

	/**
	 * @access protected
	 * @var string $eventValidation
	 */
	protected $eventValidation = '';

	/**
	 * @access protected
	 * @var string $userNameFieldName
	 */
	protected $userNameFieldName = 'ctl00$ContentPlaceHolder1$lcLogin$txtUserName';

	/**
	 * @access protected
	 * @var string $userNameFieldId
	 */
	protected $userNameFieldId = '_ctl0_ContentPlaceHolder1_txtUserName';

	/**
	 * @access protected
	 * @var string $passwordFieldName
	 */
	protected $passwordFieldName = 'ctl00$ContentPlaceHolder1$lcLogin$txtPassword';

	/**
	 * @access protected
	 * @var string $passwordFieldId
	 */
	protected $passwordFieldId = '_ctl0_ContentPlaceHolder1_txtPassword';

	/**
	 * @access protected
	 * @var string $submitButtonName
	 */
	protected $submitButtonName = 'ctl00$ContentPlaceHolder1$lcLogin$btnSubmit';

	/**
	 * @access protected
	 * @var string $submitButtonId
	 */
	protected $submitButtonId = '_ctl0:ContentPlaceHolder1:btnSubmit';
	
	/**
	 * @access protected
	 * @var string $csvUrl
	 */
	protected $csvUrl = 'http://publishers.blinkads.com/RptSubIdBreakout.aspx';
	
	/**
	 * @access protected
	 * @var string $viewState
	 */
	protected $csvViewState = '';

	/**
	 * @access protected
	 * @var string $eventValidation
	 */
	protected $csvEventValidation = 'vV/j6abFl/ADPuzgTI4RSnnlE6YouJ4ooWZ1hwFuE0n2W/q3qBL5jKasjaPbI62YOMmW/S/oy8HO5J81rvYhIR/hRYav4nO+IJB5NkA+CnXJt139VSfNcbZYTmaqLvIykUd9pUDQsZ9rv/0umt4xLxL1ntjdwrAeE1/krYlBCAcfs2f8nbqcGQ==';
	
	/**
	 * BlinkAds class constructor.
	 *
	 */
	Public Function __construct()
	{

	}
}