 ...
 * @version 0.1.2
 */
abstract class CrakRevenueAbstract Extends NetworksAbstract {
	/**
	 * @access protected
	 * @var string $loginUrl
	 */
...
//		 die;
	...
	public function getOffers()
	{
		...
		while ($Offset<1) {
			...
			$OfferRows = $HTML->find('.product');
//			print_r($OfferRows1);die;
			...
			foreach ($OfferRows as $OfferRow)
			{
				...
				$previewImage = $OfferRow->find('.preview img');
				$previewImage = $previewImage[0]->src;
				
				$Hrefs = $OfferRow->find('.name a');
				foreach ($Hrefs as $Href)
				{
					$temp = explode('=', $Href->href);
					$offerId = $temp[1];
					
					$offerName = $Href->plaintext; 
				}
				
				if (trim($offerId)=='') continue;
				
				
				$description = $OfferRow->find('.description');
				$description = $description[0]->title;
				
				
				$payout = $OfferRow->find('.payout');
				$payout = $payout[0]->plaintext;
				$payout = str_replace('&nbsp;', ' ', $payout);
				$payout = str_replace('PAYOUT:', ' ', $payout);
				$payout = str_replace("\n", ', ', $payout);
				while (stristr($payout, '  '))
				{
					$payout = str_replace('  ', ' ', $payout);
					$payout = str_replace(', , ', ', ', $payout);
				}
	...
	private function offersLogin()
	{
		...
		$Arr['login_username'] = $this->offersUsername;
		$Arr['login_password'] = $this->offersPassword;
		$Arr['action'] = 'login';
	...
	private function loadOfferPage($Offset)
	{
		...
		$arrParams = array('CURLOPT_SSL_VERIFYPEER' => FALSE,
						...
						'CURLOPT_COOKIEJAR' => $this->temp_dir() . '/cookiemonster.txt',
						'CURLOPT_COOKIEFILE' => $this->temp_dir() . '/cookiemonster.txt',
						'CURLOPT_REFERER' => 'http://www.crakrevenue.com/');
